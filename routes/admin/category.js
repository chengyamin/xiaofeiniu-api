...
router.delete('/:cid',(req,res)=>{
    ...
    pool.query('UPDATE xfn_category SET categoryId=NULL WHERE categoryId=?',req.params.cid, (err,result)=>{
        if(err)throw err;
        //至此指定类别的菜品已经修改完毕
...
router.post('/',(req,res)=>{
    ...
    pool.query('INSERT INTO xfn_category SET ?',data,(err,result)=>{//注意此处SQL语句简写
        if(err)throw err;
 ...
 * {code: 401, msg: '0 category modified, no modofication'}
 */
router.put('/',(req,res)=>{
    ...
    pool.query('UPDATE xfn_category SET ? WHERE cid=?',[data,data.cid],(err,result)=>{
        ...
        if(result.changedRows>0){//实际更新了1行
            ...
            res.send({code: 401, msg: 'no category modofication'})
        }
    })
})